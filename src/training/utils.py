...
def init_hidden(model: nn.Module, batch_size: int):
    ...
    h = torch.zeros(ModelParams.NUM_LAYERS, batch_size, ModelParams.HIDDEN_DIM, device=device)
    if isinstance(model, CharLSTM):
        c = torch.zeros(ModelParams.NUM_LAYERS, batch_size, ModelParams.HIDDEN_DIM, device=device)
        return (h, c)
    return h
